#!/usr/bin/env python3
"""Project CLI for serial, budgeted generation or editing with GPT Image models."""
...
from typing import Any, BinaryIO, Dict, Iterable, List, Never, Optional, Tuple
...
DEFAULT_CONFIG_PATH = PROJECT_ROOT / "config/imagegen.json"
DEFAULT_RUNTIME_CONFIG_PATH = PROJECT_ROOT / ".imagegen.local.json"
...
def _project_defaults() -> dict[str, object]:
    ...
    try:
        ...
        _die(f"Cannot read imagegen config: {exc}")
    if not isinstance(config, dict):
        _die("Imagegen config must be a JSON object.")
    allowed = {"model", "quality", "maxCalls"}
    unknown = sorted(set(config) - allowed)
    if unknown:
        _die(f"Imagegen config contains unsupported fields: {', '.join(unknown)}")
    if "model" in config and not isinstance(config["model"], str):
        _die("Imagegen config model must be a string.")
    if "quality" in config and not isinstance(config["quality"], str):
        _die("Imagegen config quality must be a string.")
    if "maxCalls" in config and not isinstance(config["maxCalls"], int):
        _die("Imagegen config maxCalls must be an integer.")
    return config


def _runtime_config(dry_run: bool) -> tuple[str, Path] | None:
    path = DEFAULT_RUNTIME_CONFIG_PATH
    if dry_run and not path.exists():
        return None
    if not path.is_file():
        _die("Missing .imagegen.local.json; copy the example and set local proxy values.")
    try:
        runtime = json.loads(path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc:
        _die(f"Cannot read .imagegen.local.json: {exc}")
    if not isinstance(runtime, dict) or set(runtime) != {"baseUrl", "apiKeyFile"}:
        _die(".imagegen.local.json must contain only baseUrl and apiKeyFile.")
    base_url = runtime["baseUrl"]
    key_file = runtime["apiKeyFile"]
    if not isinstance(base_url, str) or not isinstance(key_file, str):
        _die("baseUrl and apiKeyFile must be strings.")
    base_url = base_url.strip()
    key_file = key_file.strip()
    if not base_url or not key_file:
        _die("baseUrl and apiKeyFile must not be empty.")

    parsed = urlsplit(base_url)
    if parsed.scheme != "https" or not parsed.hostname or parsed.username or parsed.password:
        _die("OPENAI_BASE_URL must be an HTTPS URL without embedded credentials.")
    if parsed.query or parsed.fragment:
        _die("OPENAI_BASE_URL must not contain a query or fragment.")

    key_path = Path(key_file).expanduser()
    if not key_path.is_absolute():
        key_path = PROJECT_ROOT / key_path
    if not key_path.is_file():
        _die("apiKeyFile must point to a readable regular file.")
    if key_path.stat().st_size > 16 * 1024:
        _die("apiKeyFile is unexpectedly large.")
    return base_url.rstrip("/"), key_path


def _read_api_key(path: Path) -> str:
    key = path.read_text(encoding="utf-8").strip()
    if not key:
        _die("OPENAI_API_KEY_FILE is empty.")
    return key


def _reserve_call(ledger_path: str | None, max_calls: int, operation: str, model: str) -> None:
    if not ledger_path:
        _die("Live calls require --ledger so paid usage cannot bypass the project budget.")
    if max_calls < 1 or max_calls > HARD_MAX_CALLS:
        _die(f"--max-calls must be between 1 and {HARD_MAX_CALLS}.")
    ...
    if path.exists():
        try:
            ...
            _die(f"Cannot read call ledger: {exc}")
    else:
        ledger = {"schemaVersion": 1, "maxCalls": max_calls, "calls": []}

    calls = ledger.get("calls")
    if not isinstance(calls, list):
        _die("Call ledger has an invalid calls field.")
    if len(calls) >= max_calls:
        _die(f"Image call budget exhausted ({len(calls)}/{max_calls}).")

    # 先记账再发请求。网络中断也可能已经计费，宁可保守占用额度，不允许崩溃后静默重试超支。
    calls.append(
        {
            "number": len(calls) + 1,
            "operation": operation,
...
def _download_image(url: str) -> bytes:
    parsed = urlsplit(url)
    if parsed.scheme != "https" or not parsed.hostname or parsed.username or parsed.password:
        _die("Image response URL must be HTTPS and contain no embedded credentials.")

    hostname = parsed.hostname.lower()
    if hostname == "localhost" or hostname.endswith(".localhost"):
        _die("Image response URL uses a local hostname.")
    try:
        literal_ip = ipaddress.ip_address(hostname)
    except ValueError:
        literal_ip = None
    if literal_ip is not None and not literal_ip.is_global:
        _die("Image response URL contains a non-public IP address.")
    # 不按 DNS 解析结果拦截域名：部分代理 CDN 会把公共签名域名解析到内部边缘地址。
    # 仍拒绝显式私网 IP/localhost，并保留 HTTPS、禁止重定向、大小和图片签名边界。

    try:
        import httpx
    except ImportError:
        _die(f"URL image responses require httpx. {_dependency_hint('httpx')}")

    chunks: list[bytes] = []
    total = 0
    with httpx.stream("GET", url, follow_redirects=False, timeout=120.0) as response:
        if response.is_redirect:
            _die("Image response URL redirects are not allowed.")
        response.raise_for_status()
        declared = response.headers.get("content-length")
        if declared and int(declared) > MAX_IMAGE_BYTES:
            _die("Image response exceeds the 50MB limit.")
    ...
    if not valid:
        _die("Image response has an unsupported file signature.")
    return payload


def _result_images(result: Any) -> List[str]:
    images: List[str] = []
    for item in result.data:
        if item.b64_json:
            images.append(item.b64_json)
        elif item.url:
            images.append(base64.b64encode(_download_image(item.url)).decode("ascii"))
        else:
            _die("Image API returned neither b64_json nor url.")
    return images
...
def _ensure_outputs_available(
    outputs: List[Path], force: bool, downscale_max_dim: Optional[int], downscale_suffix: str
) -> None:
    if force:
        return
    candidates = list(outputs)
    if downscale_max_dim is not None:
        candidates.extend(_derive_downscale_path(path, downscale_suffix) for path in outputs)
    existing = [str(path) for path in candidates if path.exists()]
    if existing:
        # 在付费请求前失败；原上游脚本到响应落盘时才检查，会白白消耗一次调用。
        _die(f"Output already exists: {', '.join(existing)} (use --force to overwrite)")
...
def _create_client():
    ...
    config = _runtime_config(False)
    assert config is not None
    base_url, key_path = config
    # 显式传入代理地址，禁止 SDK 在配置缺失时回退到官方默认端点并误送代理密钥。
    return OpenAI(base_url=base_url, api_key=_read_api_key(key_path))
...
def _create_async_client():
    ...
    assert config is not None
    base_url, key_path = config
    return AsyncOpenAI(base_url=base_url, api_key=_read_api_key(key_path))
...
def _generate(args: argparse.Namespace) -> None:
    ...
    _ensure_outputs_available(
        output_paths, args.force, args.downscale_max_dim, args.downscale_suffix
    )
    ...
    _reserve_call(args.ledger, args.max_calls, "generate", args.model)
...
def _edit(args: argparse.Namespace) -> None:
    ...
    started = time.time()
    client = _create_client()
    _reserve_call(args.ledger, args.max_calls, "edit", args.model)
...
def _add_shared_args(parser: argparse.ArgumentParser, defaults: dict[str, object]) -> None:
    parser.add_argument("--model", default=defaults.get("model", DEFAULT_MODEL))
    parser.add_argument("--prompt")
    parser.add_argument("--prompt-file")
    parser.add_argument("--n", type=int, default=1)
    parser.add_argument("--size", default=DEFAULT_SIZE)
    parser.add_argument("--quality", default=defaults.get("quality", DEFAULT_QUALITY))
    ...
    parser.add_argument("--ledger", help="JSON file that reserves one entry before each paid call")
    parser.add_argument("--max-calls", type=int, default=defaults.get("maxCalls", HARD_MAX_CALLS))
...
def main() -> int:
    ...
    _add_shared_args(gen_parser, defaults)
    ...
    _add_shared_args(batch_parser, defaults)
    ...
    _add_shared_args(edit_parser, defaults)
    ...
    if args.n < 1 or args.n > 10:
        _die("--n must be between 1 and 10")
    if args.n != 1:
        _die("This project requires exactly one image per approval-gated call.")
    ...
    if args.command == "generate-batch":
        _die("generate-batch is disabled; this project requires serial, user-gated image calls.")
    if getattr(args, "downscale_max_dim", None) is not None and args.downscale_max_dim < 1:
        _die("--downscale-max-dim must be >= 1")
    if args.max_calls < 1 or args.max_calls > HARD_MAX_CALLS:
        _die(f"--max-calls must be between 1 and {HARD_MAX_CALLS}.")
