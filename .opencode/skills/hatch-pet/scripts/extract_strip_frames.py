...
def remove_chroma_background(
    ...
    for y in range(rgba.height):
        for x in range(rgba.width):
            ...
            close_to_key = color_distance(red, green, blue, chroma_key) <= threshold
            magenta_spill = (
                chroma_key[0] > 200
                and chroma_key[1] < 50
                and chroma_key[2] > 200
                and min(red, blue) >= green + 30
                and abs(red - blue) <= 80
            )
            # 生成器会在轮廓抗锯齿中留下深品红，单纯扩大欧氏阈值会误伤天蓝球衣。
            # 色相条件只清理红蓝同时压过绿色的溢色；Pet 规格已禁止使用品红本体色。
            if close_to_key or magenta_spill:
                pixels[x, y] = (0, 0, 0, 0)
    return rgba
