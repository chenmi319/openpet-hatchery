#!/usr/bin/env python3
"""Create an OpenPets run folder, prompts, and imagegen job manifest."""
...
STATE_REQUIREMENTS = {
    ...
    "waiting": [
        "Show that OpenPets needs approval, help, or user input through an expectant asking pose.",
...
def infer_description(args: argparse.Namespace, reference_paths: list[Path]) -> str:
    ...
    if args.pet_notes.strip():
        return sentence(f"A compact OpenPets pet: {args.pet_notes}")
    if args.brand_name.strip():
        return sentence(f"A compact OpenPets pet inspired by {args.brand_name}")
    if reference_paths:
        return "A compact OpenPets pet based on the provided reference image."
    return "A compact original OpenPets pet ready for animation."
...
def infer_pet_notes(args: argparse.Namespace, reference_paths: list[Path]) -> str:
    ...
    return "a compact original OpenPets pet"
...
def base_pet_prompt(args: argparse.Namespace) -> str:
    ...
    return f"""Create one clean full-body reference sprite for OpenPets pet {args.display_name}.

Pet identity: {pet_notes}.
Style: {style_contract}
{brand_block}
Place a single centered pose on a perfectly flat pure {chroma_name} {chroma_key} chroma-key background. Keep the full pet visible, compact, readable at 192x208, and easy to animate. Preserve approved reference identity cues. No scenery, unapproved text, borders, checkerboard transparency, shadows, glows, detached effects, or extra props. Keep {chroma_key} and close colors out of the pet, props, highlights, and effects."""
...
def row_prompt(
    ...
    chroma_key = args.chroma_key["hex"]
    chroma_name = args.chroma_key["name"]
    state_prompt = args.state_overrides.get(state, STATE_PROMPTS[state])
    state_requirements = "\n".join(f"- {line}" for line in STATE_REQUIREMENTS[state])
    return f"""Create one horizontal animation strip for OpenPets pet `{args.pet_id}`, state `{state}`.
...
Clean extraction: crisp opaque edges, safe padding, no scenery, unapproved text, guide marks, checkerboard, shadows, glows, motion blur, speed lines, dust, detached effects, stray pixels, or chroma-key colors inside the pet."""
...
def retry_row_prompt(
    ...
    state_prompt = args.state_overrides.get(state, STATE_PROMPTS[state])
    state_requirements = "\n".join(f"- {line}" for line in STATE_REQUIREMENTS[state])
    return f"""Create OpenPets pet row `{state}` for `{args.pet_id}`: exactly {frames} full-body frames in one horizontal strip on flat pure {chroma_name} {chroma_key}.
...
One centered complete pose per invisible slot. No unapproved text, boxes, guide marks, scenery, shadows, glows, motion blur, speed lines, dust, detached effects, stray pixels, or {chroma_key} colors in the pet."""


def make_jobs(
    run_dir: Path,
    copied_refs: list[dict[str, object]],
    allow_running_left_mirror: bool,
    ...
    for state, _row, frames, _purpose in ROWS:
        ...
        if state == "running-left" and allow_running_left_mirror:
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Require an independently generated left row for directional markings or text.",
    )
    parser.add_argument(
        "--state-overrides-file",
        default="",
        help="Optional JSON object mapping OpenPets state names to pet-specific animation actions.",
    )
    parser.add_argument(
        "--chroma-key",
        default="auto",
    ...
    if args.state_overrides_file:
        override_path = Path(args.state_overrides_file).expanduser().resolve()
        try:
            overrides = json.loads(override_path.read_text(encoding="utf-8"))
        except (OSError, json.JSONDecodeError) as exc:
            raise SystemExit(f"could not read state overrides: {exc}") from exc
        if not isinstance(overrides, dict):
            raise SystemExit("state overrides must be a JSON object")
        unknown = sorted(set(overrides) - set(STATE_PROMPTS))
        if unknown:
            raise SystemExit(f"unknown state overrides: {', '.join(unknown)}")
        if any(not isinstance(value, str) or not value.strip() for value in overrides.values()):
            raise SystemExit("every state override must be a non-empty string")
        # 宿主状态名固定，但每个 Pet 的动作语义不同；显式覆盖比事后手改生成 prompt 更可审计。
        args.state_overrides = {key: value.strip() for key, value in overrides.items()}
    ...
    jobs = {
        ...
        "run_dir": str(run_dir),
        "primary_generation_skill": "imagegen",
        # 镜像会反转号码、徽记或惯用脚；调用方必须能把这种语义风险固定进 manifest。
        "jobs": make_jobs(run_dir, copied_refs, not args.forbid_running_left_mirror),
    }
